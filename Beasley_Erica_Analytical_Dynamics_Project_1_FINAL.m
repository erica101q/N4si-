...
hold off
subplot(6,1,4);plot(t,Angles(:,4));title('Omega 1');
hold off
subplot(6,1,5);plot(t,Angles(:,5));title('Omega 2');
hold off
subplot(6,1,6);plot(t,Angles(:,6));title('Omega 3');
hold off
...
figure ()
%Plottiing
subplot(7,1,1);plot(t,Quaterions(:,1));title('Quaterion 1 vs time');xlabel('time(seconds)');ylabel('Quaterion 1')
hold off
subplot(7,1,2);
plot(t,Quaterions(:,2));
title('Quaterion 2 vs time');
xlabel('time(seconds)');
ylabel('Quaterion 2');
hold off
subplot(7,1,3);plot(t,Quaterions(:,3));title('Quaterion 3 vs Time');xlabel('time(seconds)');ylabel('Quaterion 3');
hold off
subplot(7,1,4);plot(t,Quaterions(:,4));title('Quaterion 4 vs Time');
hold off
subplot(7,1,5);plot(t,Quaterions(:,5));title('Omega 1');
hold off
subplot(7,1,6);plot(t,Quaterions(:,6));title('Omega 2');
hold off
subplot(7,1,7);plot(t,Quaterions(:,7));title('Omega 3');
...
if ~isnan(gimbalLockTime)
    fprintf('180 ambiguity detected at time t = %.2f seconds\n', gimbalLockTime);
else 
    fprintf('No 180 ambiguity detected during the simulation.\n');
end
...
circle2_x = zeros(size(theta));  % X coordinates (fixed as 0)
circle2_y = cos(theta);  % Y coordinates (for Y-axis rotation)
circle2_z = sin(theta);  % Z coordinates (for Y-axis rotation)

% Circle for rotation in the XZ-plane (for Y-axis rotation)
circle3_x = cos(theta);  % X coordinates (for Z-axis rotation)
circle3_y = zeros(size(theta));  % Y coordinates (fixed as 0)
circle3_z = sin(theta);  % Z coordinates (for Z-axis rotation)

% Create the figure for 3D animation
figure;
axis equal;
...
% Plot the first circle (in the XY-plane)
h1 = plot3(circle_x, circle_y, circle_z, 'g'); 
% Plot the second circle (in the YZ-plane)
h2 = plot3(circle2_x, circle2_y, circle2_z, 'b');
% Plot the third circle (in the XZ-plane)
h3 = plot3(circle3_x, circle3_y, circle3_z, 'm');
% Set axis limits
axis([-1 1 -1 1 -1 1]);

% Initialize the GIF filename
gif_filename = 'gimbal_lock_animation.gif';

% Time vector for animation loop
t = linspace(0, 4*pi, 300);  
h_marker = plot3(nan, nan, nan, 'ko', 'MarkerFaceColor', 'k'); 

% Tolerance for detecting near-alignment (gimbal lock condition)
tolerance = 1; 
